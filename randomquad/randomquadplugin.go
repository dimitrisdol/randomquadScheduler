// Package randomquad contains an out-of-tree plugin based on the Kubernetes
...
import (
	...
	"github.com/dimitrisdol/randomquadScheduler/randomquad/hardcoded"
...
type RandomQuadPlugin struct {
	handle framework.Handle
	model  InterferenceModel
}

var (
	_ framework.Plugin          = &RandomQuadPlugin{}
	_ framework.FilterPlugin    = &RandomQuadPlugin{}
	_ framework.ScorePlugin     = &RandomQuadPlugin{}
	_ framework.ScoreExtensions = &RandomQuadPlugin{}
)

// New instantiates a RandomQuadPlugin.
func New(configuration runtime.Object, f framework.Handle) (framework.Plugin, error) {
	return &RandomQuadPlugin{
		handle: f,
		model:  hardcoded.New(randomquadLabelKey),
	}, nil
}

// Name returns the official name of the RandomQuadPlugin.
func (_ *RandomQuadPlugin) Name() string {
	return Name
}

// findCurrentOccupants returns all Pods that are being tracked by RandomQuadPlugin
...
func (_ *RandomQuadPlugin) findCurrentOccupants(nodeInfo *framework.NodeInfo) []*corev1.Pod {
	ret := make([]*corev1.Pod, 0, 4)
	for _, podInfo := range nodeInfo.Pods {
		for key := range podInfo.Pod.Labels {
			if randomquadLabelKey == key {
...
func (ap *RandomQuadPlugin) Filter(
	ctx context.Context,
	...
	// If the given Pod does not have the randomquadLabelKey, approve it and let
	// the other plugins decide for its fate.
	if _, exists := pod.Labels[randomquadLabelKey]; !exists {
		klog.V(2).Infof("blindly approving Pod '%s/%s' as it does not have RandomQuadPlugin's label %q", pod.Namespace, pod.Name, randomquadLabelKey)
		return framework.NewStatus(framework.Success, "pod is not tracked by RandomQuadPlugin")
	}

	// For the Node at hand, find all occupant Pods tracked by RandomQuadPlugin.
	...
	// If the Node is full (i.e., 4 applications tracked by RandomQuadPlugin are
	// already scheduled on it), filter it out.
	case 4:
		klog.V(2).Infof("filtering Node %q out because 2 RandomQuadPlugin applications are already scheduled there", nodeName)
		return framework.NewStatus(framework.Unschedulable, fmt.Sprintf("Node '%s' already has 2 RandomQuadPlugin occupants", nodeName))
	...
	case 3:
		...
		if score > sla {
			msg := fmt.Sprintf("filtering Node '%s': new pod '%s/%s' ('%s') incurs huge slowdown on pod '%s/%s' ('%s')",
				nodeName, pod.Namespace, pod.Name, pod.Labels[randomquadLabelKey],)
			klog.V(2).Infof(msg)
	...
	case 1:
		...
		if score > sla {
			msg := fmt.Sprintf("filtering Node '%s': new pod '%s/%s' ('%s') incurs huge slowdown on pod '%s/%s' ('%s')",
				nodeName, pod.Namespace, pod.Name, pod.Labels[randomquadLabelKey], occ.Namespace, occ.Name, occ.Labels[randomquadLabelKey])
			klog.V(2).Infof(msg)
			return framework.NewStatus(framework.Unschedulable, msg)
		}
		fallthrough
...
//
// In the case of RandomQuadPlugin, scoring is reversed; i.e., higher score indicates
// worse scheduling decision.
// This is taken into account and "fixed" later, during the normalization.
func (ap *RandomQuadPlugin) Score(
	ctx context.Context,
	state *framework.CycleState,
	p *corev1.Pod,
	nodeName string,
	...
	// the other plugins decide for its fate.
	if _, exists := p.Labels[randomquadLabelKey]; !exists {
		klog.V(2).Infof("blindly scoring Pod '%s/%s' as it does not have RandomPlugin's label %q", p.Namespace, p.Name, randomquadLabelKey)
...
// ScoreExtensions returns the RandomQuadPlugin itself, since it implements the
// framework.ScoreExtensions interface.
func (ap *RandomQuadPlugin) ScoreExtensions() framework.ScoreExtensions {
	return ap
}
...
//
// In the case of the RandomQuadPlugin, its "Score" method produces scores of reverse
...
func (_ *RandomQuadPlugin) NormalizeScore(
	ctx context.Context,
	state *framework.CycleState,
	p *corev1.Pod,
	...
	// When no Pod (tracked by RandomQuadPlugin) is scheduled on the Node,
	// maxScore will be 0.
	if maxScore == 0 {
		for i := range scores {
