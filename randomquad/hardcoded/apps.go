// Package hardcoded contains an implementation of randomquad.InterferenceModel,
...
// HardcodedSlowDowns is an implementation of randomquad.InterferenceModel, where the
// slowdowns among all applications are known and hardcoded.
type HardcodedSlowDowns struct {
	randomquadLabelKey string
}

// New returns a new HardcodedSlowDowns with the given label key (the one that
// is used by RandomQuadPlugin to track its applications).
func New(randomquadLabelKey string) *HardcodedSlowDowns {
	return &HardcodedSlowDowns{
		randomquadLabelKey: randomquadLabelKey,
	}
}

// Attack implements randomquad.InterferenceModel; see the documentation there for
// more information.
func (m *HardcodedSlowDowns) Attack(attacker, occupant *corev1.Pod) (float64, error) {
	occPodCategory, _ := parseAppCategory(occupant.Labels[m.randomquadLabelKey])
	//  occupant   ^^^   Pod's label's value must have been
	// validated back when it was first scheduled on the Node
	newPodCategory, err := parseAppCategory(attacker.Labels[m.randomquadLabelKey])
	if err != nil {
		return -1, err
	}
...
// ToInt64Multiplier implements randomquad.InterferenceModel; see the documentation
...
// the 4 appCategories that are to be examined by the RandomQuadPlugin are as
// follows:
...
// slowDownMatrix is a type alias for internal use in RandomQuadPlugin.
